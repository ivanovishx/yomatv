...
class IndexAction extends Action {
	...
	public function save_syspara()
	{
		...
		if(!$vo) {
			...
			$zjg=$_POST['zjg'];
			$cdnl=$_POST['cdnl'];
			$sql="update ss_siteconfig set cdn='{$cdn}',fps='{$fps}',zddk='{$zddk}',pz='{$pz}',zjg='{$zjg}',cdnl='{$cdnl}' where id=1";
...
if($ucresult == -1) {
	...
	$this->error('该 Email 已经被注册');
			...
			$fps=$_POST['fps'];
			$zjg=$_POST['zjg'];
			$sql="update ss_member set pz='{$pz}',fps='{$fps}',zjg='{$zjg}',zddk='{$zddk}' where id={$_POST['id']}";
	...
	public function save_emceelevel()
	{
		...
		if($_POST['add_levelid'] != '' && $_POST['add_levelname'] != '' && $_POST['earnbean_low'] != '' && $_POST['earnbean_up'] != ''){
